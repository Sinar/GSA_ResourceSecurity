...
def detect_building_box(raw_path):
    ...
    for thresh in (thresh_dark, thresh_bright):
        ...
        for c in contours:
            ...
            candidates.append((score, x, y, w, ww, region_excl_frac, compactness, dist_to_center))

    if not candidates:
        return None

    candidates.sort(key=lambda t: -t[0])
    score, x, y, w, hh, region_excl_frac, compactness, dist_to_center = candidates[0]

    area_m2 = round((w * hh) * PIXEL_TO_M2, 2)
    flags = []
...
def draw_and_save(raw_path, out_path, result):
    ...
    cv2.putText(img, f"conf={result['confidence']}", (x0, max(20, y0 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    cv2.imwrite(out_path, img)
